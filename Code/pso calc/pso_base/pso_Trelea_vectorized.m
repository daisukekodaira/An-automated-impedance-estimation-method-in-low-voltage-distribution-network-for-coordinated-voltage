...
alpha_max = 2; % According to the paper written by Han Soohee
alpha_min = 0.01;
alpha =  ones(1,me)*NaN;
...
%       % tr = tr
       ...
       kk0 = abs(tr(i+1) - gbestval);
       kk1 = max(ergrd,abs(tr(i+1) - gbestval));
       kk2 = ergrd/(max(ergrd,abs(tr(i+1) - gbestval)));
       alpha(i) = alpha_max - (alpha_max - alpha_min)*(ergrd/max(ergrd,abs(tr(i+1))));
